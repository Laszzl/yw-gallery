...
// ═══════════════════════════════════════════════
const { DEFAULT_ITEM_DATE, DATE_MIN_YEAR, DB_NAME, DB_VERSION, DB_STORE, DB_KEY, STORAGE_KEY, CROP_JPEG_QUALITY, CROP_OUTPUT_BASE } = YW.config;
...
function renderAll({ closeModals = true } = {}) {
  if (closeModals) closeTransientModals();
  renderShell();
  renderCurrentView();
}
...
function bindEvents() {
  ...
  document.addEventListener('keydown', (event) => {
    ...
    if (!elements.datePickerModal.hidden) cancelDatePicker();
    else if (!elements.cropModal.hidden) closeCropModal(createCropResult('cancel'));
    else if (!elements.itemActionsModal.hidden) closeItemActionsModal();
    else closePhotoManageModal();
  });
...
initApp().catch((err) => console.error('应用初始化失败:', err));
  ...
  Object.assign(YW.state, { state, viewState, normalizeState, serializeState });
  Object.assign(YW.storage, { saveState, saveStateStrict, scheduleSave, loadState, exportData, importData });
  Object.assign(YW.data, { createPerson, updatePersonPhoto, deletePerson, createGroup, deleteGroup, createCategory, deleteCategory, createItem, deleteItem, updateItemPhotos });
  Object.assign(YW.dom, { elements, cacheElements });
  Object.assign(YW.modals, { showModal, openItemActionsModal, closeItemActionsModal, closePhotoManageModal, openPhotoManageModal, openGalleryManageModal });
  Object.assign(YW.crop, { showCropModal, closeCropModal, cropFile, cropInputFiles });
  Object.assign(YW.render, { renderShell, renderAll, renderCurrentView, showHomeView, showAddView, showAthleteView, showSettingsView, refreshItemCard, removeItemCard });
  Object.assign(YW.forms, { withFormLock, syncFileSummaries, syncDateDisplay });
  Object.assign(YW.railMask, { updateRailMask, scheduleRailMask, setupRailMasks });
